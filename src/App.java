...
public class App {
    ...
    */
    ...
    static void localizarPedidosPremium() {
        ...
        // Busca binária no vetor ordenado crescentemente por valorFinal().
        // Objetivo: encontrar o PRIMEIRO índice cujo valorFinal() >= valorCorte.
        // Complexidade: O(log n) — evita varredura completa do vetor.
        int inicio = 0;
        int fim = quantPedidos - 1;
        int primeiroPremium = quantPedidos; // sentinela: nenhum encontrado

        while (inicio <= fim) {
            int meio = inicio + (fim - inicio) / 2;
            if (pedidosOrdenadosPorValor[meio].valorFinal() >= valorCorte) {
                primeiroPremium = meio; // candidato; tenta encontrar um índice menor
                fim = meio - 1;
            } else {
                inicio = meio + 1;
            }
        ...
        // Itera apenas o sufixo elegível — O(k), onde k é a quantidade de premium
        if (primeiroPremium == quantPedidos) {
            System.out.println("Nenhum pedido premium encontrado para o valor de corte R$ "
                    + String.format("%.2f", valorCorte) + ".");
        } else {
            System.out.println("Pedidos premium (valor final >= R$ " + String.format("%.2f", valorCorte) + "):\n");
            for (int i = primeiroPremium; i < quantPedidos; i++) {
