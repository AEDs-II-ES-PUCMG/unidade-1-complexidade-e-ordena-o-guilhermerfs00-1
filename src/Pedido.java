...
public class Pedido implements Comparable<Pedido>{
	...
	/** Cache do valor final calculado, para evitar recálculo desnecessário sobre os itens */
	private double valorFinalCache = -1;

	/** Indica se o cache de valorFinal está válido */
	...
	public boolean incluirProduto(Produto novo, int quantidade) {
		...
		if (quantItens < MAX_ITENS) {
			...
			cacheValido = false; // invalida o cache ao modificar o pedido
			return true;
		}
		return false;
	}
	...
	public double valorFinal() {
		...
		// Retorna o valor cacheado se ainda for válido — evita reiterações desnecessárias
		if (cacheValido) {
			return valorFinalCache;
		}
		...
		valorPedidoBD = new BigDecimal(valorPedido).setScale(2, RoundingMode.HALF_UP);
		valorFinalCache = valorPedidoBD.doubleValue();
		cacheValido = true;
    ...
    public double valorCatalogoAtual() {
        ...
        return new BigDecimal(soma).setScale(2, RoundingMode.HALF_UP).doubleValue();
    }
    ...
    public double indiceEconomia() {
        return new BigDecimal(valorCatalogoAtual() - valorFinal()).setScale(2, RoundingMode.HALF_UP).doubleValue();
    }
